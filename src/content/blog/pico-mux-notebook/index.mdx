...
My reasoning and process for designing my first custom PCB for the Raspberry Pico microcontroller
...
Amongst all these were the required components to allow them to work properly, pullup-resistors, and decoupling capacitors.
...
Some things to note, the pullup-resistors needed for the all the SDA and SCL pins were moved to where the STEMMA/QWIIC ports are defined, which
...
the pullup-resistors, and pullup capacitors.
...
placement of the traces in that section. This also includes all the necessary parts for it to run properly, decoupling capacitor and pull up resistors.
...
The SD card reader is positioned in the right side of the board, with all of its pullup-resisors being placed around the general area. 
...
internal pullup-resistors on the Pico W, it wouldn't be nearly enough to maintain a consistent connection. 
...
The reason for adding jumpers was to just have them in case I ever needed them. After already needing to another PCB, from the pullup resistor incident, 
...
### `PCB`
The main changes are, again, the addition of the missing pullup-resistors, jumpers for the TCA9548a, and changing the Neopixel footprint.

![Final PCB Pullup-Resistors and Jumpers](assets/final-pcb-pullup.png)

The first image shows the missing pullup resistors at the bottom, and the new jumpers at the top. The way the jumpers work is by just adding some solder to the jumper 
